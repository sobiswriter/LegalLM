...
- **AI-Powered Analysis**: Instantly generate concise, structured summaries, identify potential legal risks, and analyze key clauses within your documents.
...
- **Modern & Fully Responsive UI**: A clean, modern user interface built for an efficient and pleasant user experience on both desktop and mobile devices. The layout intelligently adapts to smaller screens for easy navigation and analysis on the go.
...
3.  **Set up environment variables:**
    Create a file named `.env.local` in the root of your project and add your Google AI API key. You can get a key from [Google AI Studio](https://aistudio.google.com/app/apikey).
...
2. Remove the tracked `.env` locally and make sure it's ignored by git (the repo already ignores `.env.local`).
...
Notes:
- The Genkit AI flows run within the Next.js server environment and do not require a separate server when deployed, simplifying the process.
